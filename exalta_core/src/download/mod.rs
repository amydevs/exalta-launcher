...
use reqwest::{Url, Method, header::{HeaderMap, HeaderValue}};
...
static BUILD_URL: Lazy<Url> = Lazy::new(|| Url::parse("https://rotmg-build.decagames.com/").unwrap());
...
pub async fn request_checksums(build_hash: &str, platform: &str) -> Result<ChecksumFiles, Box<dyn std::error::Error>> {
    ...
    let resp = CLIENT.request(Method::GET, url).headers(defheaders).send().await?;
